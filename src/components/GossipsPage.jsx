...
// Single Gossip View - Full page for one gossip (Improved UI matching general theme)
const SingleGossipView = ({ gossip, currentUser, onBack, onVote, onDelete, onCommentAdded }) => {
  const [showCommentBox, setShowCommentBox] = useState(false);
  const [newComment, setNewComment] = useState('');
  const [loading, setLoading] = useState(false);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-white dark:from-gray-950 dark:via-purple-950/20 dark:to-gray-950 animate-fade-in">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-purple-100 text-sm mt-1">
                Posted by <span className="font-semibold">{gossip.author}</span> • {new Date(gossip.createdAt).toLocaleString()}
              </p>
            </div>
            {(isAuthor || isAdmin) && (
              <button
                onClick={() => onDelete(gossip.id)}
                className="p-2 text-white hover:bg-red-500/30 rounded-xl transition-all hover:scale-110 active:scale-95"
                title="Delete gossip"
              >
                <Trash2 className="w-5 h-5" />
              </button>
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        {/* Gossip Content Card - Enhanced Design */}
        <div className="bg-white dark:bg-gray-800/70 rounded-2xl border-2 border-gray-200 dark:border-gray-700 hover:border-purple-300 dark:hover:border-purple-700 hover:shadow-2xl transition-all duration-300 overflow-hidden animate-scale-in hover-lift">
          <div className="flex gap-4 p-6">
            {/* Vote Section */}
            <div className="flex flex-col items-center gap-3 min-w-[60px]">
              <button
                onClick={() => onVote(gossip.id, hasUpvoted ? null : 'up')}
                className={`p-3 rounded-xl transition-all transform hover:scale-110 active:scale-95 shadow-md ${
                  hasUpvoted 
                    ? 'bg-gradient-to-br from-green-500 to-green-600 text-white shadow-green-500/50' 
                    : 'bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 text-gray-600 dark:text-gray-400 hover:from-green-50 hover:to-green-100 dark:hover:from-green-900/20 dark:hover:to-green-800/20'
                }`}
              >
                <ThumbsUp className="w-5 h-5" />
              </button>
              <span className={`font-bold text-2xl px-3 py-1 rounded-lg ${
                score > 0 
              ...
              <button
                ...
                className={`p-3 rounded-xl transition-all transform hover:scale-110 active:scale-95 shadow-md ${
                  hasDownvoted 
                    ...
                    : 'bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-800 text-gray-600 dark:text-gray-400 hover:from-red-50 hover:to-red-100 dark:hover:from-red-900/20 dark:hover:to-red-800/20'
                }`}
              >
                <ThumbsDown className="w-5 h-5" />
              </button>
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <span className="font-bold text-gray-900 dark:text-white text-lg">{gossip.author}</span>
          ...
          {/* Action Bar with Gradient */}
          <div className="border-t border-gray-200 dark:border-gray-700 px-6 py-4 bg-gradient-to-r from-purple-50/50 to-pink-50/50 dark:from-purple-900/10 dark:to-pink-900/10">
            <div className="flex items-center gap-6 text-sm">
              <button
                onClick={() => setShowCommentBox(!showCommentBox)}
                className="flex items-center gap-2 text-gray-700 dark:text-gray-300 hover:text-purple-600 dark:hover:text-purple-400 font-semibold transition-all hover:scale-105"
              >
                <MessageCircle className="w-5 h-5" />
                Comment
              </button>
              <button className="flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 font-medium transition-all hover:scale-105">
                <span>Share</span>
              </button>
              <button className="flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 font-medium transition-all hover:scale-105">
                <span>Report</span>
              </button>
              <div className="ml-auto flex items-center gap-2 px-4 py-2 bg-white dark:bg-gray-800 rounded-full shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        {/* Add Comment Box - Enhanced */}
        {showCommentBox && (
          <div className="bg-white dark:bg-gray-800/70 rounded-2xl border-2 border-purple-300 dark:border-purple-700 p-6 shadow-xl animate-scale-in hover-lift">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="font-semibold text-gray-900 dark:text-white">Comment as {currentUser.username}</span>
            </div>
            <textarea
              placeholder="💭 What are your thoughts?"
              value={newComment}
              onChange={(e) => setNewComment(e.target.value)}
              className="w-full p-4 border-2 border-gray-300 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-400 transition-all resize-none bg-white dark:bg-gray-900 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 shadow-sm"
              rows={4}
              maxLength={300}
              autoFocus
            />
            <div className="flex items-center justify-between mt-4">
              <span className={`text-sm font-medium ${newComment.length > 270 ? 'text-orange-600' : 'text-gray-500 dark:text-gray-400'}`}>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="px-5 py-2 text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition-all font-medium"
                >
                  Cancel
                </button>
                <button
                  onClick={handleAddComment}
                  disabled={!newComment.trim() || loading}
                  className="px-6 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl hover:from-purple-700 hover:to-pink-700 hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed font-semibold flex items-center gap-2 transform hover:scale-105 active:scale-95"
                >
                  {loading ? (
                    <>
        ...
        <div className="bg-white dark:bg-gray-800/70 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-lg hover-lift">
          ...
          {commentTree.length === 0 ? (
            <div className="text-center py-16 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/10 dark:to-pink-900/10 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-700">
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm mb-4">Be the first to share your thoughts!</p>
              <button
                onClick={() => setShowCommentBox(true)}
                className="px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all transform hover:scale-105 active:scale-95"
              >
                Add First Comment
              </button>
            </div>
